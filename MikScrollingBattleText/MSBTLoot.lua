...
local function HandleCurrency(parserEvent)
	...
	if IsClassic then
		...
		itemName, numAmount, itemTexture, _, _, totalMax, _, itemQuality = C_CurrencyInfo.GetCurrencyInfoFromLink(itemLink)
	end
...
local function HandleItems(parserEvent)
	...
	local numLooted = parserEvent.amount or 1
	local numItems = C_Item.GetItemCount(itemLink) or 0
